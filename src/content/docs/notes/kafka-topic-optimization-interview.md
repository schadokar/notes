...
```
1,000 accounts × 10 flows × 10 log types = 100,000 topics
```
...
- Broker restarts take **45+ minutes** (metadata reload).
- Controller failover takes **8+ minutes**.
...
graph TD
    ...
    C -->|"ZK watches grow O(n)<br>Controller failover: 8+ min"| FAIL[Delayed Failover]
    ...
    CG -->|"Group coordinator manages<br>offset commits per partition"| LAG[Offset Commit Lag]
...
At 100K topics, this takes **45+ minutes**. At 1M topics, your broker restarts would take **hours**. Rolling upgrades become a multi-day affair.
...
> 💡 **Staff-level insight:** KRaft mode (KIP-500) eliminates ZooKeeper but doesn't magically solve topic explosion. The KRaft controller still maintains an in-memory metadata log. At millions of partitions, the controller's snapshot size and log replay time become the bottleneck — just a different bottleneck. KRaft raises the ceiling from ~200K to ~2-5M partition-replicas, but it doesn't remove the ceiling.
...
If each account has its own consumer group (a common pattern with topic-per-tenant), you have:

```
1,000 accounts × (some number of consumer groups per account)
```

The `__consumer_offsets` topic stores offset commits for all groups. With 1,000+ consumer groups, each committing offsets for multiple partitions, the `__consumer_offsets` topic becomes a hotspot. The group coordinator broker handles all join/sync/heartbeat requests — at scale, this becomes a bottleneck.
...
```
partitions = max(target throughput (MB/s) / throughput per partition (MB/s), desired parallelism)
```

A single partition can handle **10-50 MB/s** on modern hardware. For `logs.metric` with 10K accounts × 10 flows × 50 msg/s × 5KB avg = ~25 GB/s aggregate... actually, let's be precise:

```
10,000 accounts × 10 flows × 50 msg/s × 5 KB = 25,000,000 KB/s = ~24 GB/s
```

That's enormous. At 50 MB/s per partition, you'd need 480 partitions just for `logs.metric`. In practice, not all accounts are active simultaneously — apply a **concurrency factor** of 10-20%. Realistic throughput: ~2.5-5 GB/s → 50-100 partitions.
...
| ---------------------------------- | --------------------------------- | ------------------------------------- |
| Max partition-replicas (practical) | ~200K                             | ~2-5M                                 |
| Controller failover                | 8+ min at 100K topics             | Seconds (Raft leader election)        |
...
But KRaft is not a silver bullet. Even with KRaft, **2-5M partition-replicas is all you'll realistically manage**. At our consolidated design (700 partitions × 3 replicas = 2,100 partition-replicas), we're well within limits even at 10,000x growth.
...
**The fix**: Compound key sharding (see code in Section 3.9). Detect large tenants dynamically based on throughput metrics and add shard suffixes.
...
*24-week migration timeline. Staff engineers present timelines, not just designs. This shows you understand the operational reality.*
...
- [Wix Engineering: Building a Log Platform with Kafka](https://www.wix.engineering/) — Multi-tenant log processing at 200M+ sites.
- [Datadog: Kafka at Scale](https://www.datadoghq.com/blog/) — Topic consolidation and multi-tenant operations.
- [Confluent Blog: Running Kafka at Scale](https://www.confluent.io/blog/) — Operational best practices for large Kafka deployments.
- [LinkedIn Engineering: Kafka at LinkedIn](https://engineering.linkedin.com/blog/topic/kafka) — Where Kafka was born, scaling to trillions of messages/day.
- [Uber Engineering: Kafka Multi-Tenancy](https://www.uber.com/blog/engineering/) — Uber's approach to multi-tenant Kafka clusters.

### Conference Talks

- "Operating Kafka at Scale" — various talks at Kafka Summit and QCon covering 10K+ topic operations.
- "Multi-Tenant Kafka: Lessons Learned" — Kafka Summit presentations from Confluent engineers on topic consolidation patterns.
...
2. **Write a multi-tenant producer/consumer** using the Sarama code from Section 3.9. Add per-tenant metrics. Build a Grafana dashboard showing per-tenant throughput and lag.
